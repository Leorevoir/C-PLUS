...
{
    for (u64 i = 0; i < self->file_count; ++i) {
        const CPlusFile *file = &self->files[i];
        u64 j = 0;
        u64 line = 1;
        u64 column = 1;
...
static __inline void lexer_open_read_streams(IOStream *stream)
{
...
static __inline void lexer_load_inputs(Lexer *self, const Array *array)
{
    CPlusFile *files = self->files;

    array_foreach(array, const char *, it, {
        files[__i].stream.filename = *it;
        files[__i].tokens = new (ArrayClass, sizeof(CPlusToken), 32);
        lexer_open_read_streams(&files[__i].stream);
    });
}
...
static void lexer_ctor(void *instance, va_list *args)
{
    ...
    self->file_count = array->size(array);
    self->output.stream.filename = va_arg(*args, const char *);
    self->flags = va_arg(*args, int);

    allocate(self->files, self->file_count * sizeof(CPlusFile));
    lexer_load_inputs(self, array);
}
...
static void lexer_dtor(void *instance)
{
    ...
    for (u32 i = 0; i < self->file_count; ++i) {
        if (self->files[i].stream.fd > 0) {
            close(self->files[i].stream.fd);
            self->files[i].stream.fd = -1;
        }
    }
}
